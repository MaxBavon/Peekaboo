...
__all__ = ["Assets", "TILE_SIZE", "PIXEL_RATIO", "UNIT", "COLORS"]
...
class Assets:
    ...
    @classmethod
    def load_dir_anim_sheet(cls, path):
        animation = []
        for filename in os.listdir(path):
            animation.append(cls.load_image(os.path.join(path, filename)))
        return animation
    ...
    def load_rec_sprites(cls, path, sprites):
        for directory in os.listdir(path):
            ...
            if os.path.isdir(new_path): # Load next directory
                ...
                elif "sheet" in directory:
                    sprites[directory] = cls.load_dir_anim_sheet(os.path.join(new_path))
                else:
    ...
    def load_ui(cls):
        ...
        for (root, dirs, files) in os.walk(path, topdown=True):
            for file_ in files:
                ui[file_[:-4]] = cls.load_image(root + '/' + file_, UI_PIXEL_RATIO)
        return ui
